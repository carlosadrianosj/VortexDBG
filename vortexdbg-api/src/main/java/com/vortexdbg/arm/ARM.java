...
public class ARM {
        ...
        for (int reg : regs) {
            ...
            switch (reg) {
                ...
                case Arm64Const.UC_ARM64_REG_LR: {
                    VortexdbgPointer lr = VortexdbgPointer.register(emulator, Arm64Const.UC_ARM64_REG_LR);
                    builder.append(String.format(Locale.US, "\nLR=%s", lr));
                    break;
                }
    ...
    private static void appendMemoryDetails32(Emulator<?> emulator, Instruction ins, capstone.api.arm.OpInfo opInfo, boolean thumb, StringBuilder sb) {
        ...
        if (op.length == 2 &&
            ...
            if (mem.getIndex() == 0 && mem.getScale() == 1 && mem.getLshift() == 0) {
                VortexdbgPointer base = VortexdbgPointer.register(emulator, ins.mapToUnicornReg(mem.getBase()));
            ...
            if (mem.getIndex() > 0 && mem.getScale() == 1 && mem.getLshift() == 0 && mem.getDisp() == 0 &&
                ...
                VortexdbgPointer base = VortexdbgPointer.register(emulator, ins.mapToUnicornReg(mem.getBase()));
                long base_value = base == null ? 0L : base.peer;
                VortexdbgPointer index = VortexdbgPointer.register(emulator, ins.mapToUnicornReg(mem.getIndex()));
        ...
        if ("ldrd".equals(ins.getMnemonic()) && op.length == 3 &&
            ...
            if (mem.getIndex() == 0 && mem.getScale() == 1 && mem.getLshift() == 0) {
                ...
                VortexdbgPointer base = VortexdbgPointer.register(emulator, regId);
                long base_value = base == null ? 0L : base.peer;
                addr = base_value + mem.getDisp();
    ...
    public static void initArgs(Emulator<?> emulator, boolean padding, Number... arguments) {
        ...
        while (!list.isEmpty()) {
            ...
            VortexdbgPointer pointer = memory.allocateStack(emulator.getPointerSize());
            assert pointer != null;
            if (emulator.is64Bit()) {
                if ((pointer.peer % 8) != 0) {
    ...
    public static VortexdbgPointer adjust_ip(VortexdbgPointer ip) {
        int adjust = 4;
