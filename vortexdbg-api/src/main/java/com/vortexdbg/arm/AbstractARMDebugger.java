...
public abstract class AbstractARMDebugger implements Debugger {
    ...
    final boolean handleWriteCommand(Backend backend, String line) {
        ...
        if (command.startsWith("wb0x") || command.startsWith("ws0x") || command.startsWith("wi0x") || command.startsWith("wl0x")) {
            ...
            Pointer pointer = VortexdbgPointer.pointer(emulator, addr);
            if (pointer != null) {
    ...
    final boolean handleMemoryCommand(String line) {
        ...
        if (reg != -1) {
            Pointer pointer = VortexdbgPointer.register(emulator, reg);
            if (pointer != null) {
    ...
    private void searchStack(byte[] data) {
        ...
        VortexdbgPointer stack = emulator.getContext().getStackPointer();
        Backend backend = emulator.getBackend();
    ...
    @Override
    public final void brk(VortexdbgPointer pc, int svcNumber) {
        if (pc != null) {
    ...
    private void printMcpConfig(int port, int index) {
        String serverName = index == 0 ? "vortexdbg-mcp-server" : "vortexdbg-mcp-server-" + index;
