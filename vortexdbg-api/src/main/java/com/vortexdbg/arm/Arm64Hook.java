...
public abstract class Arm64Hook extends Arm64Svc {
    ...
    @Override
    public final VortexdbgPointer onRegister(SvcMemory svcMemory, int svcNumber) {
        byte[] code;
        if (enablePostCall) {
        ...
        String name = getName();
        VortexdbgPointer pointer = svcMemory.allocate(code.length, name == null ? "Arm64Hook" : name);
