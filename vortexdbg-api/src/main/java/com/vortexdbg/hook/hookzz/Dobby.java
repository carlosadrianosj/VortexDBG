...
public final class Dobby extends BaseHook implements IHookZz {
    ...
    public void replace(long functionAddress, Svc svc) {
        ...
        int ret = dobbyHook.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), callback, originCall).intValue();
        if (ret != RT_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
    ...
    public void replace(long functionAddress, ReplaceCallback callback, boolean enablePostCall) {
        ...
        int ret = dobbyHook.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), replaceCall, originCall).intValue();
        if (ret != RT_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
    ...
    public <T extends RegisterContext> void instrument(long functionAddress, final InstrumentCallback<T> callback) {
        ...
        int ret = dobbyInstrument.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), dbiCall).intValue();
        if (ret != RT_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
