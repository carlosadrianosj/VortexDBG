...
public final class HookZz extends BaseHook implements IHookZz {
    ...
    public void replace(long functionAddress, Svc svc) {
        ...
        int ret = zzReplace.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), callback, originCall).intValue();
        if (ret != RS_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
    ...
    public void replace(long functionAddress, ReplaceCallback callback, boolean enablePostCall) {
        ...
        int ret = zzReplace.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), replaceCall, originCall).intValue();
        if (ret != RS_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
    ...
    public <T extends RegisterContext> void wrap(long functionAddress, final WrapCallback<T> callback) {
        ...
        int ret = zzWrap.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), preCall, postCall).intValue();
        if (ret != RS_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
    ...
    public <T extends RegisterContext> void instrument(long functionAddress, final InstrumentCallback<T> callback) {
        ...
        int ret = zzDynamicBinaryInstrumentation.call(emulator, VortexdbgPointer.pointer(emulator, functionAddress), dbiCall).intValue();
        if (ret != RS_SUCCESS) {
            throw new IllegalStateException("ret=" + ret);
        }
    }
