...
public class VortexdbgPointer extends Pointer implements PointerArg {

    private static final Logger log = LoggerFactory.getLogger(VortexdbgPointer.class);
    ...
    public static long nativeValue(Pointer ptr) {
        ...
        VortexdbgPointer up = (VortexdbgPointer) ptr;
    ...
    VortexdbgPointer(Emulator<?> emulator, byte[] data) {
        super(0);
    ...
    private VortexdbgPointer(Emulator<?> emulator, long peer, int pointerSize) {
        super(0);
    ...
    public VortexdbgPointer setSize(long size) {
        if (size < 0) {
    ...
    public static VortexdbgPointer pointer(Emulator<?> emulator, long addr) {
        long peer = emulator.is64Bit() ? addr : addr & 0xffffffffL;
        return peer == 0 ? null : new VortexdbgPointer(emulator, peer, emulator.getPointerSize());
    }

    public static VortexdbgPointer pointer(Emulator<?> emulator, Number number) {
        return pointer(emulator, number.longValue());
    }

    public static VortexdbgPointer register(Emulator<?> emulator, int reg) {
    ...
    @Override
    public VortexdbgPointer getPointer(long offset) {
    ...
    public void setPointer(long offset, Pointer pointer) {
        ...
        if (pointer == null) {
            ...
            value = ((VortexdbgPointer) pointer).peer;
        }
    ...
    @Override
    public VortexdbgPointer share(long offset, long sz) {
        if (offset == 0L && sz == size) {
            return this;
        }

        VortexdbgPointer pointer = new VortexdbgPointer(emulator, peer + offset, pointerSize);
        if (size > 0) {
            if (offset > size) {
    ...
    public boolean equals(Object o) {
        ...
        return (o instanceof VortexdbgPointer) && (((VortexdbgPointer)o).peer == peer);
    }
