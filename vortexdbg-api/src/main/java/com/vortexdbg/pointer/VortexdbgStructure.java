...
public abstract class VortexdbgStructure extends Structure implements PointerArg {
     ...
     */
    private static final Pointer PLACEHOLDER_MEMORY = new VortexdbgPointer(null, null) {
        @Override
        public VortexdbgPointer share(long offset, long sz) { return this; }
    };

    public static int calculateSize(Class<? extends VortexdbgStructure> type) {
        try {
            Constructor<? extends VortexdbgStructure> constructor = type.getConstructor(Pointer.class);
    ...
    private static class ByteArrayPointer extends VortexdbgPointer {
    ...
    protected VortexdbgStructure(Emulator<?> emulator, byte[] data) {
        this(new ByteArrayPointer(emulator, data));
    }

    protected VortexdbgStructure(byte[] data) {
        this(null, data);
    }

    protected VortexdbgStructure(Pointer p) {
        super(p);
    ...
    private void checkPointer(Pointer p) {
        ...
        if (!(p instanceof VortexdbgPointer) && !isPlaceholderMemory(p)) {
            throw new IllegalArgumentException("p is NOT VortexdbgPointer");
        }
    }
    ...
    private String toString(int indent, boolean showContents, boolean dumpMemory) {
        ...
        if (!showContents) {
            ...
            if (value instanceof VortexdbgStructure) {
                value = ((VortexdbgStructure)value).toString(indent + 1, !(value instanceof Structure.ByReference), dumpMemory);
            }
            contents.append("=");
