...
open class UnicornBackend @Throws(BackendException::class) internal constructor(
    ...
    init {
        try {
            this.unicorn = Unicorn(if (is64Bit) UnicornConst.UC_ARCH_ARM64 else UnicornConst.UC_ARCH_ARM, UnicornConst.UC_MODE_ARM)
    ...
    override fun reg_read(regId: Int): Number {
        try {
            return unicorn.reg_read(regId) as Number
        } catch (e: UnicornException) {
            throw BackendException(e)
        }
    }
    ...
    override fun reg_write(regId: Int, value: Number) {
        try {
            unicorn.reg_write(regId, value.toLong())
        } catch (e: UnicornException) {
            throw BackendException(e)
        }
    }
    ...
    override fun hook_add_new(callback: CodeHook, begin: Long, end: Long, user_data: Any?) {
        try {
            val unHook = unicorn.hook_add(object : unicorn.CodeHook {
                override fun hook(u: Unicorn, address: Long, size: Int, user: Any?) {
                    callback.hook(this@UnicornBackend, address, size, user)
                }
            }, begin, end, user_data)
            callback.onAttach(object : UnHook {
                override fun unhook() {
                    unHook.unhook()
                }
            })
    ...
    override fun debugger_add(callback: DebugHook, begin: Long, end: Long, user_data: Any?) {
        try {
            val unHook = unicorn.debugger_add(object : unicorn.DebugHook {
                override fun onBreak(u: Unicorn, address: Long, size: Int, user: Any?) {
                    callback.onBreak(this@UnicornBackend, address, size, user)
                }
    ...
    override fun hook_add_new(callback: ReadHook, begin: Long, end: Long, user_data: Any?) {
        try {
            val unHook = unicorn.hook_add(object : unicorn.ReadHook {
                override fun hook(u: Unicorn, address: Long, size: Int, user: Any?) {
                    callback.hook(this@UnicornBackend, address, size, user)
                }
            }, begin, end, user_data)
    ...
    override fun hook_add_new(callback: WriteHook, begin: Long, end: Long, user_data: Any?) {
        try {
            val unHook = unicorn.hook_add(object : unicorn.WriteHook {
                override fun hook(u: Unicorn, address: Long, size: Int, value: Long, user: Any?) {
                    callback.hook(this@UnicornBackend, address, size, value, user)
                }
            }, begin, end, user_data)
            callback.onAttach(object : UnHook {
                override fun unhook() {
    ...
    private fun hookEventMem(callback: EventMemHook, type: Int, user_data: Any?, unmappedType: EventMemHook.UnmappedType) {
        try {
            val map = unicorn.hook_add(object : unicorn.EventMemHook {
                override fun hook(u: Unicorn, address: Long, size: Int, value: Long, user: Any?): Boolean {
                    return callback.hook(this@UnicornBackend, address, size, value, user, unmappedType)
                }
            }, type, user_data)
    ...
    override fun hook_add_new(callback: InterruptHook, user_data: Any?) {
        try {
            unicorn.hook_add(object : unicorn.InterruptHook {
                override fun hook(u: Unicorn, intno: Int, user: Any?) {
                    val swi = decodeSWI(emulator, this@UnicornBackend, is64Bit)
                    callback.hook(this@UnicornBackend, intno, swi, user)
                }
            }, user_data)
        } catch (e: UnicornException) {
            throw BackendException(e)
    ...
    override fun hook_add_new(callback: BlockHook, begin: Long, end: Long, user_data: Any?) {
        try {
            val unHook = unicorn.hook_add(object : unicorn.BlockHook {
                override fun hook(u: Unicorn, address: Long, size: Int, user: Any?) {
                    callback.hookBlock(this@UnicornBackend, address, size, user)
                }
            }, begin, end, user_data)
            callback.onAttach(object : UnHook {
                override fun unhook() {
