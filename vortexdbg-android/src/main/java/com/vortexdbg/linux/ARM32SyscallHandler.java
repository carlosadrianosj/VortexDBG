...
public class ARM32SyscallHandler extends AndroidSyscallHandler {
    ...
    public void hook(Backend backend, int intno, int swi, Object user) {
        ...
        VortexdbgPointer pc = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_PC);
        final int bkpt;
        if (pc == null) {
            bkpt = swi;
        } else {
    ...
    private int mremap(Emulator<?> emulator) {
        Arm32RegisterContext context = emulator.getContext();
        VortexdbgPointer old_address = context.getR0Pointer();
        int old_size = context.getR1Int();
        int new_size = context.getR2Int();
        int flags = context.getR3Int();
        VortexdbgPointer new_address = context.getR4Pointer();
        if (log.isDebugEnabled()) {
    ...
    protected int ptrace(Emulator<?> emulator) {
        ...
        Pointer addr = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R2);
        Pointer data = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R3);
        log.info("ptrace request=0x{}, pid={}, addr={}, data={}", Integer.toHexString(request), pid, addr, data);
        return 0;
    }

    private int utimes(Emulator<?> emulator) {
        Pointer filename = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        Pointer times = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        if (log.isDebugEnabled()) {
    ...
    private int utimensat(Backend backend, Emulator<?> emulator) {
        ...
        Pointer pathname = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        Pointer times = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R2);
    ...
    private int unlink(Emulator<?> emulator) {
        Pointer pathname = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
    ...
    private int pipe(Emulator<?> emulator) {
        Pointer pipefd = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
    ...
    private int set_tls(Backend backend, Emulator<?> emulator) {
        VortexdbgPointer tls = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        if (log.isDebugEnabled()) {
    ...
    private int cacheflush(Backend backend, Emulator<?> emulator) {
        Pointer begin = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        Pointer end = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
    ...
    private int pthread_clone(Emulator<?> emulator) {
        ...
        VortexdbgPointer child_stack = context.getPointerArg(1);
        ...
        VortexdbgPointer fn = child_stack.getPointer(0);
        child_stack = child_stack.share(4, 0);
        VortexdbgPointer arg = child_stack.getPointer(0);
        child_stack = child_stack.share(4, 0);
    ...
    private int bionic_clone(Emulator<?> emulator) {
        ...
        VortexdbgPointer fn = context.getR5Pointer();
        VortexdbgPointer arg = context.getR6Pointer();
        List<String> list = new ArrayList<>();
        if ((flags & CLONE_VM) != 0) {
            list.add("CLONE_VM");
        }
    ...
    private int llseek(Backend backend, Emulator<?> emulator) {
        ...
        Pointer result = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R3);
    ...
    private int stat64(Emulator<AndroidFileIO> emulator) {
        Pointer pathname = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        Pointer statbuf = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        String path = FilenameUtils.normalize(pathname.getString(0), true);
        if (log.isDebugEnabled()) {
    ...
    private int newselect(Backend backend, Emulator<?> emulator) {
        ...
        Pointer readfds = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        Pointer writefds = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R2);
        Pointer exceptfds = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R3);
        Pointer timeout = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R4);
        int size = (nfds - 1) / 8 + 1;
        if (log.isDebugEnabled()) {
    ...
    private int getpeername(Backend backend, Emulator<?> emulator) {
        int sockfd = backend.reg_read(ArmConst.UC_ARM_REG_R0).intValue();
        Pointer addr = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        Pointer addrlen = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R2);
        if (log.isDebugEnabled()) {
    ...
    private int poll(Backend backend, Emulator<?> emulator) {
        Pointer fds = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
    ...
    private int syslog(Backend backend, Emulator<?> emulator) {
        ...
        Pointer bufp = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
    ...
    private int lgetxattr(Backend backend, Emulator<?> emulator) {
        Pointer path = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        Pointer name = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        Pointer value = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R2);
    ...
    private int reboot(Backend backend, Emulator<?> emulator) {
        ...
        Pointer arg = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R3);
        if (log.isDebugEnabled()) {
    ...
    private int recvfrom(Emulator<?> emulator) {
        Backend backend = emulator.getBackend();
        int sockfd = backend.reg_read(ArmConst.UC_ARM_REG_R0).intValue();
        Pointer buf = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        int len = backend.reg_read(ArmConst.UC_ARM_REG_R2).intValue();
        int flags = backend.reg_read(ArmConst.UC_ARM_REG_R3).intValue();
        Pointer src_addr = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R4);
        Pointer addrlen = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R5);
    ...
    private int sendto(Backend backend, Emulator<?> emulator) {
        ...
        int len = backend.reg_read(ArmConst.UC_ARM_REG_R2).intValue();
        int flags = backend.reg_read(ArmConst.UC_ARM_REG_R3).intValue();
        Pointer dest_addr = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R4);
    ...
    private int getsockopt(Backend backend, Emulator<?> emulator) {
        ...
        Pointer optval = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R3);
        Pointer optlen = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R4);
        if (log.isDebugEnabled()) {
    ...
    private int getgroups(Backend backend, Emulator<?> emulator) {
        ...
        Pointer list = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        if (log.isDebugEnabled()) {
    ...
    protected int uname(Emulator<?> emulator) {
        Pointer buf = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        if (log.isDebugEnabled()) {
        ...
        release.setString(0, "1.0.0-vortexdbg");
    ...
    private int munmap(Backend backend, Emulator<?> emulator) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("munmap start=0x{}, length={}, offset={}, from={}", Long.toHexString(start), length, System.currentTimeMillis() - timeInMillis, VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR));
        }
        return 0;
    }
    ...
    private int prctl(Backend backend, Emulator<?> emulator) {
        ...
        switch (option) {
            ...
            case PR_SET_DUMPABLE:
                return 0;
            case PR_SET_NAME: {
                Pointer threadName = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
            ...
            case PR_GET_NAME: {
                ...
                Pointer buffer = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
            ...
            case BIONIC_PR_SET_VMA:
                ...
                Pointer pointer = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R4);
                if (log.isDebugEnabled()) {
    ...
    protected int clock_gettime(Backend backend, Emulator<?> emulator) {
        ...
        Pointer tp = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
    ...
    private int writev(Backend backend, Emulator<?> emulator) {
        int fd = backend.reg_read(ArmConst.UC_ARM_REG_R0).intValue();
        Pointer iov = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
    ...
    private int mmap2(Backend backend, Emulator<?> emulator) {
        ...
        if (log.isDebugEnabled() || warning) {
            String msg = "mmap2 start=0x" + Long.toHexString(start) + ", length=" + length + ", prot=0x" + Integer.toHexString(prot) + ", flags=0x" + Integer.toHexString(flags) + ", fd=" + fd + ", offset=" + offset + ", from=" + VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR);
            if (warning) {
                log.warn(msg);
                if (log.isDebugEnabled()) {
    ...
    private int gettimeofday(Emulator<?> emulator) {
        Pointer tv = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        Pointer tz = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        return gettimeofday(emulator, tv, tz);
    }

    private int faccessat(Backend backend, Emulator<AndroidFileIO> emulator) {
        int dirfd = backend.reg_read(ArmConst.UC_ARM_REG_R0).intValue();
        Pointer pathname_p = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
    ...
    private int fstatat64(Backend backend, Emulator<AndroidFileIO> emulator) {
        int dirfd = backend.reg_read(ArmConst.UC_ARM_REG_R0).intValue();
        Pointer pathname = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
        Pointer statbuf = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R2);
        int flags = backend.reg_read(ArmConst.UC_ARM_REG_R3).intValue();
        String path = FilenameUtils.normalize(pathname.getString(0), true);
        if (log.isDebugEnabled()) {
    ...
    private int open(Emulator<AndroidFileIO> emulator) {
        ...
        String pathname = pathname_p.getString(0);
        String msg = "open pathname=" + pathname + ", oflags=0x" + Integer.toHexString(oflags) + ", mode=" + Integer.toHexString(mode) + ", from=" + VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR);
        if (log.isDebugEnabled()) {
            log.debug(msg);
        }
        int fd = open(emulator, pathname, oflags);
    ...
    private int getdents64(Emulator<AndroidFileIO> emulator) {
        ...
        VortexdbgPointer dirp = context.getPointerArg(1);
    ...
    private int fstat(Backend backend, Emulator<?> emulator) {
        int fd = backend.reg_read(ArmConst.UC_ARM_REG_R0).intValue();
        Pointer stat = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_R1);
