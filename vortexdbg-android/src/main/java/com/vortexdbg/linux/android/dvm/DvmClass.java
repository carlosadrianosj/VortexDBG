...
public class DvmClass extends DvmObject<Class<?>> {
    ...
    final Map<String, VortexdbgPointer> nativesMap = new HashMap<>();

    public final VortexdbgPointer findNativeFunction(Emulator<?> emulator, String method) {
        VortexdbgPointer fnPtr = nativesMap.get(method);
        ...
        if (fnPtr == null) {
            for (Module module : emulator.getMemory().getLoadedModules()) {
                ...
                if (symbol != null) {
                    fnPtr = (VortexdbgPointer) symbol.createPointer(emulator);
                    break;
                }
            }
        }
