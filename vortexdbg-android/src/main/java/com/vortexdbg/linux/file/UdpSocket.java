...
public class UdpSocket extends SocketIO implements FileIO {
    ...
    private int getIFaceList(Emulator<?> emulator, long argp) {
        try {
            ...
            IFConf conf = IFConf.create(emulator, VortexdbgPointer.pointer(emulator, argp));
            Pointer ifcu_req = VortexdbgPointer.pointer(emulator, conf.getIfcuReq());
    ...
    protected int getIFaceFlags(Emulator<?> emulator, long argp) {
        IFReq req = IFReq.createIFReq(emulator, VortexdbgPointer.pointer(emulator, argp));
        req.unpack();
