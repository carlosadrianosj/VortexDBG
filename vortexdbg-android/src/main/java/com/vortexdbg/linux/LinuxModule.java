...
public class LinuxModule extends Module {
    ...
    static LinuxModule createVirtualModule(String name, final Map<String, VortexdbgPointer> symbols, Emulator<?> emulator) {
        if (symbols.isEmpty()) {
            throw new IllegalArgumentException("symbols is empty");
        }

        List<VortexdbgPointer> list = new ArrayList<>(symbols.values());
        list.sort((o1, o2) -> (int) (o1.peer - o2.peer));
        VortexdbgPointer first = list.get(0);
        VortexdbgPointer last = list.get(list.size() - 1);
        ...
        LinuxModule module = new LinuxModule(base, base, size, name, null,
            ...
            public Symbol findSymbolByName(String name, boolean withDependencies) {
                VortexdbgPointer pointer = symbols.get(name);
                if (pointer != null) {
        ...
        for (Map.Entry<String, VortexdbgPointer> entry : symbols.entrySet()) {
    ...
    public int callEntry(Emulator<?> emulator, String... args) {
        ...
        final VortexdbgPointer stack = memory.allocateStack(0);
        ...
        VortexdbgPointer kernelArgumentBlock = memory.allocateStack(emulator.getPointerSize());
        assert kernelArgumentBlock != null;
        kernelArgumentBlock.setInt(0, argc);

        if (log.isDebugEnabled()) {
            VortexdbgPointer sp = memory.allocateStack(0);
