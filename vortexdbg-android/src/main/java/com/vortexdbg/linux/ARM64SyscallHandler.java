...
public class ARM64SyscallHandler extends AndroidSyscallHandler {
    ...
    public void hook(Backend backend, int intno, int swi, Object user) {
        ...
        VortexdbgPointer pc = VortexdbgPointer.register(emulator, Arm64Const.UC_ARM64_REG_PC);
    ...
    private int bionic_clone(Emulator<?> emulator) {
        ...
        VortexdbgPointer fn = context.getPointerArg(5);
        VortexdbgPointer arg = context.getPointerArg(6);
        List<String> list = new ArrayList<>();
        if ((flags & CLONE_VM) != 0) {
            list.add("CLONE_VM");
        }
    ...
    private long mremap(Emulator<?> emulator) {
        Arm64RegisterContext context = emulator.getContext();
        VortexdbgPointer old_address = context.getXPointer(0);
        int old_size = context.getXInt(1);
        int new_size = context.getXInt(2);
        int flags = context.getXInt(3);
        VortexdbgPointer new_address = context.getXPointer(4);
        if (log.isDebugEnabled()) {
    ...
    private int gettimeofday(Emulator<?> emulator) {
        Pointer tv = VortexdbgPointer.register(emulator, Arm64Const.UC_ARM64_REG_X0);
        Pointer tz = VortexdbgPointer.register(emulator, Arm64Const.UC_ARM64_REG_X1);
    ...
    private int fstat(Backend backend, Emulator<?> emulator) {
        int fd = backend.reg_read(Arm64Const.UC_ARM64_REG_X0).intValue();
        Pointer stat = VortexdbgPointer.register(emulator, Arm64Const.UC_ARM64_REG_X1);
    ...
    private int read(Backend backend, Emulator<?> emulator) {
        int fd = backend.reg_read(Arm64Const.UC_ARM64_REG_X0).intValue();
        Pointer buffer = VortexdbgPointer.register(emulator, Arm64Const.UC_ARM64_REG_X1);
