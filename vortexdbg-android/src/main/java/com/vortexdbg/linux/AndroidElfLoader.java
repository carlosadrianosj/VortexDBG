...
public class AndroidElfLoader extends AbstractLoader<AndroidFileIO> implements Memory, Loader {
    ...
    private VortexdbgPointer initializeTLS(String[] envs) {
        ...
        auxv.setPointer(0, VortexdbgPointer.pointer(emulator, AT_RANDOM));
        auxv.setPointer(emulator.getPointerSize(), __stack_chk_guard);
        final int AT_PAGESZ = 6;
        auxv.setPointer(emulator.getPointerSize() * 2L, VortexdbgPointer.pointer(emulator, AT_PAGESZ));
        auxv.setPointer(emulator.getPointerSize() * 3L, VortexdbgPointer.pointer(emulator, ARMEmulator.PAGE_ALIGN));
        ...
        final VortexdbgPointer argv = allocateStack(0x100);
        assert argv != null;
        ...
        final VortexdbgPointer tls = allocateStack(0x80 * 4); // tls size
        assert tls != null;
    ...
    private final VortexdbgPointer environ;
    ...
    private LinuxModule loadInternal(LibraryFile libraryFile) throws IOException {
        ...
        for (MemoizedObject<ElfRelocation> object : dynamicStructure.getRelocations()) {
            ...
            Pointer relocationAddr = VortexdbgPointer.pointer(emulator, load_base + relocation.offset());
            assert relocationAddr != null;

            Logger log = LoggerFactory.getLogger("com.vortexdbg.linux." + soName);
            if (log.isDebugEnabled()) {
        ...
        if (executable) {
            ...
            if (count > 0) {
                VortexdbgPointer pointer = VortexdbgPointer.pointer(emulator, load_base + dynamicStructure.getPreInitArrayOffset());
                if (pointer == null) {
                    throw new IllegalStateException("DT_PREINIT_ARRAY is null");
                }
                for (int i = 0; i < count; i++) {
                    VortexdbgPointer ptr = pointer.share((long) i * emulator.getPointerSize(), 0);
        ...
        if (elfFile.file_type == ElfFile.FT_DYN) { // not executable
            ...
            if (count > 0) {
                VortexdbgPointer pointer = VortexdbgPointer.pointer(emulator, load_base + dynamicStructure.getInitArrayOffset());
                if (pointer == null) {
    ...
    @Override
    public Module loadVirtualModule(String name, Map<String, VortexdbgPointer> symbols) {
