...
public class ArmExIdx {
    ...
    public Frame arm_exidx_step(Emulator<?> emulator, Unwinder unwinder, Module module, long fun, DwarfCursor context) {
        ...
        if (fun == entry) { // first instruction of function
            ...
            VortexdbgPointer fp = VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_SP);
        ...
        if (compact) { // android_external_libunwind/src/arm/Gex_tables.c
            ...
            VortexdbgPointer pointer = VortexdbgPointer.pointer(emulator, module.base + addr);
            assert pointer != null;
            ...
            case ARM_EXIDX_CMD_REG_POP: {
                ...
                for (int m = 0; m < 16; m++) {
                    if ((edata.data & (1 << m)) != 0) {
                        ...
                        VortexdbgPointer sp = VortexdbgPointer.pointer(emulator, context.cfa);
                        assert sp != null;
