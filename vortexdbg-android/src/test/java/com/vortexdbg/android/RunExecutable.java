...
class RunExecutable {
    ...
    static void run(File executable, ModuleListener listener, String[] preloads, String...args) throws IOException {
        try (Emulator<?> emulator = AndroidEmulatorBuilder.for32Bit()
            ...
            if (environ != null) {
                Pointer pointer = VortexdbgPointer.pointer(emulator, libc.base + environ.value);
                assert pointer != null;
                System.err.println("environ=" + pointer + ", value=" + pointer.getPointer(0));
            }
            Number __errno = libc.callFunction(emulator, "__errno");
            Pointer pointer = VortexdbgPointer.pointer(emulator, __errno.intValue() & 0xffffffffL);
            assert pointer != null;
...
//             emulator.traceCode();
            Pointer strerror = VortexdbgPointer.pointer(emulator, libc.callFunction(emulator, "strerror", UnixEmulator.ECONNREFUSED).intValue() & 0xffffffffL);
