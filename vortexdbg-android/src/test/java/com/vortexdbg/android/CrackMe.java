...
public class CrackMe {
    ...
    public CrackMe() {
        executable = new File("vortexdbg-android/src/test/resources/example_binaries/crackme1");
    ...
    private void crack() {
        ...
        ixHook.register(executable.getName(), "strlen", new ReplaceCallback() {
            ...
            public HookStatus onCall(Emulator<?> emulator, long originFunction) {
                ...
                System.err.printf("strlen[\"%s\"] called from %s%n", str, VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR));
        ...
        ixHook.register(executable.getName(), "puts", new ReplaceCallback() {
            ...
            public HookStatus onCall(Emulator<?> emulator, long originFunction) {
                String str = emulator.getContext().getPointerArg(0).getString(0);
                System.err.printf("puts[\"%s\"] called from %s%n", str, VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR));
        ...
        ixHook.register(executable.getName(), "memcpy", new ReplaceCallback() {
            ...
            public HookStatus onCall(Emulator<?> emulator, long originFunction) {
                ...
                Inspector.inspect(src.getByteArray(0, size), "memcpy dest=" + dest + ", src=" + src + ", LR=" + VortexdbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR));
                return HookStatus.RET(emulator, originFunction);
            }
        });
        ixHook.refresh();
